...
//					System.out.println("------------------------------");
	...
	public void GuideLine(Mat immagine) { //가이드 선 
...
//		System.loadLibrary(Core.NATIVE_LIBRARY_NAME);
		...
		try { //핸드트레킹 및 캡을 이용하기위한 opencv 실행 팡리 불러오기
		...
		while (!close) {
			if (!webcam.isOpened() && !close) { // 에러 처리 
				...
				modifica = v.filtromorfologico(2, 7, v.filtrocolorehsv(0, 0, 0, 180, 255, 40, mimm)); //색 인식

				difetti = v.inviluppodifetti(mimm, v.cercacontorno(mimm, modifica, false, false, 300, 5000), false, 5); //손인식을 위한 화면의 색, 위치 인식
...
//				}
				if (count != 0) {
					if (buffer.size() < 7) {
						...
						centro = v.filtromediamobile(buffer, v.centropalmo(mimm, difetti)); //중심점 잡기
...
//					v.disegnaditacentropalmo(mimm, centro, dito, difetti);
					subpoint = v.handDraw(centro, dita); // 세부적인 손 표현

					v.disegnaditacentropalmo(mimm, centro, dito, dita, subpoint); //손, 박스 표시
					center = centro; //센터위치 확인
					
					v.connect(centro); // 마우스 및 키보드 연결
//					v.mousetrack(dita, dito, centro, r, true, mimm, temp);

				}
				v.GuideLine(mimm); // 가이드 선 표시
				v.frametolabel(mimm); // 화면에 창 보이게 
			}
		}
	}
}