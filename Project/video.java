...
//		Imgproc.erode(modifica, modifica, Imgproc.getStructuringElement(Imgproc.MORPH_ELLIPSE, new Size(ke,ke)));
	...
	public void disegnaditacentropalmo(Mat immagine, Point center, Point dito, List<Point> dita) {
		...
		// Core.putText(immagine, center.toString(), center, Core.FONT_HERSHEY_COMPLEX,
		// 1, new Scalar(0,200,255));
...
//				stringa = "Blocco puntatore: attendo azione!";
				stringa = "Pointer Block: Waiting for action!";
				r.mouseRelease(InputEvent.BUTTON1_DOWN_MASK);
...
//				stringa = "Attendo azione!";
				stringa = "I look forward to your action!";
		...
		while (true && !close) {
			...
			if (!webcam.isOpened() && !close) {
				...
				if (!close) {
					...
					// modifica = v.filtromorfologico(2, 7, v.filtrocolorergb(0, 0, 0, 40, 40, 40,
					// mimm));
